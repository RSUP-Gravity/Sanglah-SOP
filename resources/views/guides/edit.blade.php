...
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
            <h3 class="font-bold text-gray-900">Edit Data Panduan</h3>
        </div>
        
        <form action="{{ route('guides.update', $guide) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <div class="relative">
                         <select name="chapter" id="chapter" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent appearance-none bg-white">
                            <option value="Umum" {{ old('chapter', $guide->chapter) == 'Umum' ? 'selected' : '' }}>Umum</option>
                            <option value="Penting" {{ old('chapter', $guide->chapter) == 'Penting' ? 'selected' : '' }}>Penting</option>
                            <option value="Teknis" {{ old('chapter', $guide->chapter) == 'Teknis' ? 'selected' : '' }}>Teknis</option>
                        </select>
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" id="description" rows="5"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">{{ old('description', $guide->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if($guide->image_path)
                        <div class="mb-2">
                            <img src="{{ Storage::url($guide->image_path) }}" class="h-24 w-auto rounded-lg object-cover" alt="Current Image">
                        </div>
                    @endif
                ...
                <div>
                    ...
                    <input type="url" name="video_url" id="video_url" value="{{ old('video_url', $guide->video_url) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                    @error('video_url')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" min="0" value="{{ old('sort_order', $guide->sort_order) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent">
                </div>

                <div class="flex items-center pt-8">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $guide->is_active) ? 'checked' : '' }}>
