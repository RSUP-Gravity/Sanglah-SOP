...
<div class="min-h-screen bg-white">
    ...
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col lg:flex-row min-h-[calc(100vh-4rem)]">
            <!-- Sidebar Navigation -->
            <aside class="w-full lg:w-64 flex-shrink-0 lg:border-r border-gray-200 py-6 pr-6 lg:sticky lg:top-16 lg:h-[calc(100vh-4rem)] overflow-y-auto">
                <nav class="space-y-6">
                    ...
                    @empty
                        <p class="text-sm text-gray-500">Belum ada panduan tersedia.</p>
                    @endforelse
                </nav>
            </aside>
            ...
            <main class="flex-1 lg:pl-10 py-8 max-w-4xl">
                <!-- Welcome State if no specific guide selected (list view acting as index) -->
                <div class="prose prose-green max-w-none">
                    <h1>Selamat Datang di Pusat Panduan SOP</h1>
                    <p class="text-xl text-gray-600">
                        Temukan dokumentasi lengkap mengenai penggunaan Sistem Informasi Manajemen Standar Operasional Prosedur (SOP) di RSUP Sanglah.
                    </p>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 not-prose mt-8">
                        ...
                        <div class="bg-white border rounded-xl p-6 shadow-sm hover:shadow-md transition">
                            <h2 class="text-lg font-bold text-gray-900 mb-4">{{ $chapterName }}</h2>
                            <ul class="space-y-2">
                                @foreach($guidesInChapter->take(4) as $item)
                                <li>
                                    <a href="{{ route('guides.showPublic', $item->slug) }}" class="flex items-center text-gray-600 hover:text-green-600">
